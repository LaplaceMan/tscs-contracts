...
const main = async () => {
    ...
    const ZIMU = await ethers.getContractFactory("ZMToken");
    const zimu = await ZIMU.deploy(tscsAddress, deployerAddress, "0x21e19e0c9bab2400000", deployerAddress);
  ...
  console.log("stAddress", stAddress);
    ...
    const tx4 = await tscsExemple.setSettlementStrategy(0, onetime0Address, 'OT0');
    const tx5 = await tscsExemple.setSettlementStrategy(1, divide1Address, 'DI1');
    const tx6 = await tscsExemple.setSettlementStrategy(2, onetime2Address, 'OTM2');
    // 主合约设置代币合约地址
